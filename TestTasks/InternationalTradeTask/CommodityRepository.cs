...
namespace TestTasks.InternationalTradeTask
{
    public class CommodityRepository
    {
        ...
        {
            return GetTariff(commodityName, getImport: true);
        }

        public double GetExportTariff(string commodityName)
        {
            return GetTariff(commodityName, getImport: false);
        }

        private double GetTariff(string commodityName, bool getImport)
        {
            foreach (var root in _allCommodityGroups)
            {
                try
                {
                    return CommoditySearchHelper.GetEffectiveTariff(root, commodityName, getImport);
                }
            ...
            throw new ArgumentException($"Commodity with name \"{commodityName}\" not found.");
        }
